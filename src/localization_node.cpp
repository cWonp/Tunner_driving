...
private:
    ...
    void setGoal(const geometry_msgs::PoseStamped::ConstPtr& click_msg) //Event : pcw
    {
        ROS_INFO("%d th goal is set", goal_count_);
        vector<geometry_msgs::PoseStamped>().swap(goal_set_);
    ...
    void EventEndCallback(const std_msgs::Empty::ConstPtr &msg){ //Event : pcw
        event_flag = false;
        set_event_around_goal = false;
    }
    int FindSection(geometry_msgs::PoseStamped pos){ //Event : pcw
        for(int i = 0; i < map_section.size(); i++){
            if((pos.x <= map_section[i].start_x_ && pos.x > map_section[i].end_x_) ||
                    (pos.x > map_section[i].start_x_ && pos.x <= map_section[i].end_x_) &&
                    (pos.y <= map_section[i].start_y_ && pos.y > map_section[i].end_y_) ||
                    (pos.y > map_section[i].start_y_ && pos.y <= map_section[i].end_y_)){
                int event_section = map_section[i].idx_;
                return event_section;
            }
        }
    }
    double CalcPosErr(geometry_msgs::PoseStamped pos1, geometry_msgs::PoseStamped pos2){ //Event : pcw
        double err_x = pos1.x - pos2.x;
        double err_y = pos1.y - pos2.y;
    ...
    void poseCallback(const std_msgs::Int32::ConstPtr &empty_pose_msg)
    {
        ...
        else
        {
            ...
            if(event_flag){
                if(set_event_goal){
                    static int event_section, previous_goal_section, current_pos_section = 0;
                    event_section = FindSection(goal_set_[0].pose.position);
                    previous_goal_section = FindSection(current_goal_.pose.position);
                    current_pos_section = FindSection(pose_msg.pose.pose.position);

                    if(!((event_section > current_pos_section && event_section <= previous_goal_section) ||
                            (event_section >= previous_goal_section && event_section < current_pos_section))){
                        behavior_cnt = 1;
                    }
                    if(event_section == current_pos_section){
                        if(CalcPosErr(goal_set_[0].pose.position, current_goal_.pose.position) > CalcPosErr(pose_msg.pose.pose.position, current_goal_.pose.position)){
                            behavior_cnt = 1;
                        }
                    }
                    ...
                    set_event_goal = false;
                }

                if(global_dist_err < config_.global_dist_boundary_){
                    set_event_around_goal = true;
                }
            }
            else{
        ...
        //Event : pcw
        static double previous_x = 0, previous_y = 0;
        static double idx = 0;
        static bool done_flag = false;

        if(!done_flag && sqrt(pow(previous_x-pose_msg.pose.pose.position.x,2)+pow(previous_y-pose_msg.pose.pose.position.y,2)) > section_length){
            map_range map_range_;
            map_range_.start_x_ = previous_x;
            map_range_.start_y_ = previous_y;
            map_range_.end_x_ = pose_msg.pose.pose.position.x;
            map_range_.end_y_ = pose_msg.pose.pose.position.y;
            map_range_.idx_ = idx++;

            map_section.push(map_range_);

            previous_x = pose_msg.pose.pose.position.x;
            previous_y = pose_msg.pose.pose.position.y;
        }
        ...
        //----------------------------------------------- postech mode
            ...
            case 0://moving to go turning point: start straight
                ...
                //                }
                if(event_flag && set_event_around_goal){ //Event : pcw
                    if(fabs(global_dist_err) > fabs(around_event_goal_lenth)){
                        Next_step=false;
                        behavior_cnt++;
                        postech_mode = STOP_MODE;
                    }
...
private:
    ...
    bool event_flag =false;//For a event goal //Event : pcw // new_goal_flag->event_flag
    bool set_event_goal = false; //Event : pcw
    bool set_event_around_goal = false; //Event :pcw
    ...
    int around_event_goal_lenth = 50;
    //event : pcw
